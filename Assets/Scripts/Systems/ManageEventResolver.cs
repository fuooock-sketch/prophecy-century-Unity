...
namespace ProphecyCentury.Systems
{
    public sealed class ManageEventResolver
    {
        ...
        private void HandleTalent(RunState runState, BoardUnitState owner, SkillDefinition talent, string eventType, UnitCardState target, string reason, UnitCardState source, int eventValue, HashSet<string> processed, int depth)
        {
            switch (talent.kind)
            {
                case "while_on_board_on_entry_race_self_gain_attack":
                    if (eventType == "on_entry" && target != owner && CountsAsRace(target, talent.race))
                    {
                ...
                case "while_on_board_on_entry_tagged_units_gain_attack":
                    if (eventType == "on_entry" && target != owner && CountsAsRace(target, talent.entryRace))
                    {
                ...
                case "on_entry_random_race_units_gain_power":
                    ...
                    {
                        PickRandom(runState.boardUnits.Where(unit => unit != owner && CountsAsRace(unit, talent.race)).ToList(), Count(talent, owner))
                ...
                case "on_leave_retrigger_random_race_entry_effects":
                    if (eventType == "on_leave" && target == owner)
                    {
                        ...
                        var candidates = runState.boardUnits
                            .Where(unit => unit != owner && CountsAsRace(unit, talent.race) && !excluded.Contains(unit.unitId) && HasEntryTalent(unit))
                            .ToList();
                ...
                case "on_entry_race_units_devour_shop_gain_attack":
                    if (eventType == "on_entry" && target == owner)
                    {
                        foreach (var unit in runState.boardUnits.Where(unit => CountsAsRace(unit, talent.race)).ToList())
                        {
        ...
        private void RetriggerRaceRoundEndTalents(RunState runState, BoardUnitState owner, SkillDefinition talent, HashSet<string> processed, int depth)
        {
            ...
            var targets = runState.boardUnits.Where(unit => unit != owner && CountsAsRace(unit, talent.race)).ToList();
        ...
        private void DevourShopCard(RunState runState, BoardUnitState owner, SkillDefinition talent, bool highestAttack, HashSet<string> processed, int depth)
        {
            ...
            if (stats != null)
            {
                AddStat(runState, owner, "hp", EffectiveHp(card) * stats.hp, owner, processed, depth);
                AddStat(runState, owner, "attack", EffectiveAttack(card) * stats.attack, owner, processed, depth);
                AddStat(runState, owner, "defense", EffectiveDefense(card) * stats.defense, owner, processed, depth);
                AddStat(runState, owner, "power", EffectivePower(card) * stats.power, owner, processed, depth);
                AddStat(runState, owner, "speed", EffectiveSpeed(card) * stats.speed, owner, processed, depth);
                AddStat(runState, owner, "morale", EffectiveMorale(card) * stats.morale, owner, processed, depth);
            }
            else
            {
        ...
        private static bool CountsAsRace(UnitCardState unit, string race)
        {
        ...
        private static int CountRace(RunState runState, string race, string fallbackRace)
        {
            ...
            return runState.boardUnits.Count(unit => CountsAsRace(unit, targetRace));
        }
