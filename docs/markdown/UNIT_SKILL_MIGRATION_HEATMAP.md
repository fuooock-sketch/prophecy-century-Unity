...
- `unit_data.xlsx` 增加隐藏召唤单位“幻影”，不补回已废弃的“精锐羽卫”。
- 驯兽师、兽骑兵按“格尔兽默认数量”同步表格文案。
- 格尔兽吞噬数量按“商店中当前数量”同步表格文案。
- 土元素战斗技能从 Excel 表移除。
- 魔法龙改为按场上最高火元素数量召唤，金色为 120%，至少为 10。
- 旧数量成长术语统一改为“获得数量”。
...
| --- | --- | --- |
| 旧数量成长术语 | 🟩 | 必须统一改成“获得数量”。 |
| 百分比获得数量文本 | 🟩 | 发现后需转成明确数量来源或设计确认。 |
| 默认数量除法公式 | 🟧 | 格尔兽已改为按商店当前数量计算，仍需专项验收。 |
